...
function parseAndValidateLink(raw) {
    ...
    if (!input) {
        return { ok: false, error: "Please paste a link." };
    }
    ...
    // Require https for everything (per spec)
    ...
    // --- Direct audio ---
    // Allow querystrings after the extension
    const full = input.toLowerCase();
    const allowed = [".mp3", ".m4a", ".wav"];
    const hasAllowedExt = allowed.some((ext) => full.includes(ext) && full.indexOf(ext) < full.length);

    // Safer check: pathname ends with ext
    ...
    // If they pasted a signed URL where the extension is in query, you can relax later.
    return {
        ok: false,
        error:
...
export default async function handler(req, res) {
    try {
        ...
        // Fetch card including hash + lockout fields
        ...
        // If already claimed, require change-link endpoint instead
        if (card.claimed_at) {
            return res.status(409).json({
                error: "Already claimed",
                message: "This card has already been claimed. Use “Change link” to update it.",
            });
        }
        ...
        // Successful claim: store link + mark claimed + reset attempts/lock
        const claimUpdate = {
        ...
        if (updateErr) {
            return res.status(500).json({ error: "Failed to claim card", details: updateErr });
        }
